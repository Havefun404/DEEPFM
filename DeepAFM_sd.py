...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, attention, valid_dimension, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size,
                 learning_rate, lamda_bilinear, keep_prob, optimizer_type, batch_norm, activation_function, verbose, early_stop, feature_size,
                 field_size, random_seed=2017):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.attention:
                ...
                self.attention_mul = tf.reshape(
                    tf.add(tf.matmul(tf.reshape(self.element_wise_product, shape=[-1, self.hidden_factor]), \
                                     self.weights['attention_W']), self.weights['attention_b']),
                    shape=[-1, int(num_interactions), 10])
                # p*h*relu(Wx+b)
                self.attention_relu = tf.reduce_sum(
                    tf.multiply(tf.multiply(tf.nn.relu(self.attention_mul), self.weights['attention_h']),
                                self.weights['attention_p']), 2, keep_dims=True)
                # softmax
            ...
            if self.attention:
                ...
                # self.AFM = tf.matmul(self.AFM, self.weights['attention_p'])
            ...
            for i in range(0, len(self.layers)):
                self.deep = tf.add(tf.matmul(self.deep, self.weights['layer_%d' % i]),
            ...
            # _________DeepAFM_________
            if self.attention:
                self.y_bias = self.weights['bias'] * tf.ones_like(self.train_labels)
                self.AFM = tf.add(self.AFM, self.y_bias)
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    ...
                    for i in range(len(self.layers)):
                        self.loss = tf.nn.l2_loss(
                            tf.subtract(self.train_labels, self.out)) + tf.contrib.layers.l2_regularizer(
                            self.lamda_bilinear)(self.weights['layer_%d' % i])
                else:
    ...
    def _initialize_weights(self):
        ...
        all_weights['feature_embeddings'] = tf.Variable(
            tf.random_normal([self.feature_size, self.hidden_factor], 0.0, 0.01),
            name='feature_embeddings')  # features_M * K
        all_weights['feature_bias'] = tf.Variable(tf.random_normal([self.feature_size, 1], 0.0, 1.0),
        ...
        if self.attention:
            ...
            all_weights['attention_W'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.hidden_factor, 10)), dtype=np.float32,
                name="attention_W")
        ...
        if num_layer > 0:
            ...
            all_weights['layer_0'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(input_size, self.layers[0])), dtype=np.float32)
            ...
            all_weights['prediction'] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(in_size, 1)),
                                                    dtype=np.float32)  # layers[-1] * 1
        else:
            all_weights['prediction'] = tf.Variable(
                np.ones((self.hidden_factor * 2 + 1, 1), dtype=np.float32))  # hidden_factor * 1
        return all_weights
    ...
    def partial_fit(self, Xi, Xv, y):  # fit a batch
        ...
        feed_dict = {self.feat_index: Xi, self.feat_value: Xv, self.train_labels: y, self.dropout_keep: self.keep_prob,
    ...
    def evaluate(self, Xi, Xv, y):  # evaluate the results for an input set
        ...
        while len(Xi_batch) > 0:
            ...
            feed_dict = {self.feat_index: Xi_batch, self.feat_value: Xv_batch, self.train_labels: y_batch,
        ...
        if self.loss_type == 'square_loss':
            ...
            predictions_bounded = np.minimum(predictions_bounded,
                                             np.ones(len(y)) * max(y_true))  # bound the higher values
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "Neural FM: dataset=%s, attention=%s, valid_dimension=%s, hidden_factor=%d, dropout_keep=%s, layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d, feature_size=%d, field_size=%d"
            % (args.dataset, args.attention, args.valid_dimension, args.hidden_factor, args.keep_prob, args.layers,
               args.loss_type, args.pretrain, args.epoch, args.batch_size, args.lr, args.lamda, args.optimizer,
               args.batch_norm, args.activation, args.early_stop, feature_size, field_size))
    ...
    print('best_epoch:', best_epoch + 1, 'train_loss:', model.train_rmse[best_epoch], 'validation_loss:',
          model.valid_rmse[best_epoch], 'test_loss:', model.test_rmse[best_epoch])
